...
starting_urls <- c(
  ...
  "https://www.earwolf.com/show/how-did-this-get-played/", # HDTGP
...
walk(starting_urls, function(start_url) {
  ...
  # Append to Rda file, then export to CSV
  load(file = "data/earwolf_podcasts.Rda")
  ...
  earwolf_podcasts %>%
    ...
    select(-html) %>% # Drop the column that contains the HTML data
